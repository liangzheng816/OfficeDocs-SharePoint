...
>[!NOTE]
> Normally, when the source permissions are inherited, the migrated destination permissions will also be inherited. However, in special cases where the destination permission is changed to explicit while the source remains inherited, the destination permissions will not be overwritten in this mode.
