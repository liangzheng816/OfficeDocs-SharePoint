...
> [!NOTE]
> These PowerShell cmdlets aren't currently available for users of Office 365 operated by 21Vianet in China.
...
- **[Register-SPMTMigration](/powershell/module/spmt/register-spmtmigration)**

This cmdlet creates and then initializes a migration session. The initialization configures migration settings at the session level. If no specific setting parameters are defined, default settings are used. After a session is registered, you can add a task to the session and start migration.
...
- **[Start-SPMTMigration](/powershell/module/spmt/start-spmtmigration)**

This cmdlet starts the registered SPMT migration.

### Return the object of current session

- **[Get-SPMTMigration](/powershell/module/spmt/Get-SPMTMigration)**
...
- **[Stop-SPMTMigration](/powershell/module/spmt/Stop-SPMTMigration)**

This cmdlet cancels the current migration.
...
- **[Unregister-SPMTMigration](/powershell/module/spmt/Unregister-SPMTMigration)**

Use this cmdlet to delete the migration session.

## Sample Scenarios

**Example 1:** The IT admin adds a SharePoint on-premises task and starts migration in the background.
...
**Example 2:** The IT admin wants to bring the migration from the background "NoShow mode" to the foreground, and run below the cmdlet so the migration progress is shown in the console.

```powershell
Show-SPMTMigration
```

**Example 3:** The IT Admin wants to do a bulk migration by loading a .csv file. The sample file in this example is SPMT.csv.
