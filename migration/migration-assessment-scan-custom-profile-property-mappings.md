...
This scan output provides you with a list of profile property mappings that aren't included in the target environment. This  information enables you to make a decision on the direction moving forward.
...
This scan report contains a list of all the profile property mappings that don't exist in the target environment. If you have multiple profile sync connections, you may see multiple mappings for a single profile property.

|Column |Description |
|-------|------------|
|ConnectionName |Name of the profile connection of which the property mapping is associated. |
|ConnectionType |Type of profile connection which helps determine from where the profile property value is being pulled. |
