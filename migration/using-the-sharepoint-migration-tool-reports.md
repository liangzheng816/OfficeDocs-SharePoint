...
The SharePoint Migration Tool generates log files, summary reports, and task level reports. This information helps you manage, audit and troubleshoot your migration process.
...
1. A file folder opens listing the task level reports that have been generated for that specific task.
...
|Total scanned items |The total number of files and list items, including those that were filtered out because of settings or potential scan issues. |
...
|Round number |The incremental number of times the report has been generated based on the multiple passes the tool made to get all the content it could into the destination. |
...
|**Column** |**Description** |
|:-----|:-----|
|Incremental round |The round number added to the end of the report name (RO, R1, etc.). It indicates if the scan or job has been rerun. |
...
|Expected migrated file count |The total number of files expected to migrate, excluding those filtered out based settings or scanned potential issues. |
